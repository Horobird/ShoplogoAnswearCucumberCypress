...
const svgToPath = (svg) => svg.match(/<path\s+d="([^"]*)/)[1];
const titleToSlug = (title) => title.toLowerCase().replace(
...
const titleToHtmlFriendly = (brandTitle) => brandTitle.replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/./g, (char) => {
  const charCode = char.charCodeAt(0);
  return charCode > 127 ? `&#${charCode};` : char;
});
const htmlFriendlyToTitle = (htmlFriendlyTitle) => htmlFriendlyTitle.replace(/&#([0-9]+);/g, (_, num) => String.fromCharCode(parseInt(num))).replace(
  /&(quot|amp|lt|gt);/g,
  (_, ref) => ({ quot: '"', amp: "&", lt: "<", gt: ">" })[ref]
);
const getIconDataPath = (rootDir = getDirnameFromImportMeta(import_meta.url)) => {
  return import_node_path.default.resolve(rootDir, "_data", "simple-icons.json");
};
const getIconsDataString = (rootDir = getDirnameFromImportMeta(import_meta.url)) => {
  return import_promises.default.readFile(getIconDataPath(rootDir), "utf8");
};
const getIconsData = async (rootDir = getDirnameFromImportMeta(import_meta.url)) => {
...
const getThirdPartyExtensions = async (readmePath = import_node_path.default.join(
  ...
  module2 = module2.split(
    module2.includes("<picture>") ? "<picture>" : '<img src="'
  )[0];
  return {
    module: {
      name: /\[(.+)\]/.exec(module2)[1],
